...
/**
 * Convert time values to hours based on the selected unit
 */
function convertToHours(value, unit) {
  switch (unit) {
    ...
    case 'shifts':
      return value * 8;  // Assume 8-hour shifts
    default:
...
function calculateBenefits(roadmap, benchmarks, fin_inputs) {
  ...
  if (fin_inputs) {
    ...
    // Convert time values to hours for calculations
    const downtimeHours = convertToHours(fin_inputs.downtime_hours, fin_inputs.time_unit || 'hrs');
    const productionHours = convertToHours(fin_inputs.production_hours, fin_inputs.time_unit || 'hrs');
    
    // Calculate cost per hour based on the selected unit
    let costPerHour = fin_inputs.downtime_cost_per_hour;
    if (fin_inputs.time_unit === 'days') {
      costPerHour = fin_inputs.downtime_cost_per_hour / 24;
    } else if (fin_inputs.time_unit === 'shifts') {
      costPerHour = fin_inputs.downtime_cost_per_hour / 8;
    }
    
    // Calculate monetary values
    if (downtimeHours && costPerHour) {
      monetaryBenefits.downtime_savings_annual = (downtimeHours * costPerHour * improvements.downtime_reduction) / 100;
    }
